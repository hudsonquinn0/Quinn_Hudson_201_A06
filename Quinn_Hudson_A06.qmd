...
[https://github.com/hudsonquinn0/Quinn_Hudson_201_A06](https://github.com/hudsonquinn0/Quinn_Hudson_201_A06)
...
```{r}
player_info[[2]][[4]] #indexing

player_info$LJ$points #naming
```
...
```{r}
player_info$MJ$height = 78 #appends player info mj with a new key for height and its corresponding value
...
```{r}
points_per_game_list = list()#initialized list

for(i in names(player_info)){
points = player_info[[i]]$points #extracts points value for each player from the list
games_played = player_info[[i]]$games_played #extracts games value for each player from the list
points_per_game_list[[i]] = points / games_played #appends points_per_game_list keys for each player with PPG value
}
...
for(i in names(points_per_game_list)){ #iterates through list
rounded_points_per_game[i] = round(points_per_game_list[[i]], 2)#rounds the ppg values and puts them in vector
}
...
```{r}
round_function = function(x){ #round function
  if(10 <= x && x< 100){
    return(round(x, -1))#round to tens if x is double digits
  }else if(100 <= x && x < 1000){
    return(round(x, -2))#rounds to 3 hundreds if 3 digits
  }else if(x >= 1000){
    return(round(x, -3))#rounds to 3 thousands if 4 digits
  }
}

rounded_mat_rand = apply(mat_rand, MARGIN = 1:2, FUN = round_function) #apply funct that uses round_function
print("ORIGINAL MATRIX") #for clarity grading the original matrix is displayed
print(mat_rand)

print("ROUNDED MATRIX")
print(rounded_mat_rand) #rounded matrix
...
```{r}
max_temps_numbers = as.data.frame(lapply(max_temps, as.numeric)) #converts all the values to numbers

avg_temps = apply(max_temps_numbers, 2, mean, na.rm = TRUE) #apply function to find the mean temps per column
...
```{r}
temps_co = subset(x = max_temps, subset = state == "CO", select = c(names(max_temps)))

temps_oh = subset(x = max_temps, subset = state == "OH", select = c(names(max_temps)))

temps_va = subset(x = max_temps, subset = state == "VA", select = c(names(max_temps)))
```
...
```{r}
temps_co_numbers = as.data.frame(apply(temps_co, 1:2, as.numeric)) #converts temps_co to numeric and back to a data frame
temps_co_numbers = temps_co_numbers[, -c(1,2)] #removes state and year columns

all_median_temps = apply(temps_co_numbers, 1, median, na.rm = TRUE) #the median temps

years = 1895:2020 #sequence to correspond to the years with median temperatures

median_temps_data_frame = data.frame(#creating a data frame with median temperatures and years
  year = years,
  median_temp = all_median_temps)

#Finds the max and median temperatures and their corresponding years and minimum and median temperatures and their corresponding years

max_median_temp = median_temps_data_frame$median_temp[which.max(median_temps_data_frame$median_temp)]

max_year = median_temps_data_frame$year[which.max(median_temps_data_frame$median_temp)]

min_median_temp = median_temps_data_frame$median_temp[which.min(median_temps_data_frame$median_temp)]

min_year = median_temps_data_frame$year[which.min(median_temps_data_frame$median_temp)]

print(paste0("The maximum median annual temperature of ", max_median_temp, " occurred in ", max_year, " and the minimum median annual temperature of ", min_median_temp, " occurred in ", min_year, "."))
```
...
sum_of_digits = function(number){
  if(number == 0){#if the number is 0 return 0
    ...
    #modulo the number to get the value of the ones place. Then floor divide the number. This eliminates the ones place which we've already accounted for. Apply the function again until you get 0
    return((number %% 10) + sum_of_digits(number %/% 10))
  }
}
result = sum_of_digits(935068) #run function with example
print(result)
```
